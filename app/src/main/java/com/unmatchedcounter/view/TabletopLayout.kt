...
import com.unmatchedcounter.util.LogUtils
...
abstract class TabletopLayoutAdapter<VH, T>(private val parent: TabletopLayout) where VH : TabletopLayoutViewHolder<T> {
    ...
    fun setPositions(positions: List<TableLayoutPosition>) {
        for (tableLayoutPosition in TableLayoutPosition.values()) {
            if (!positions.contains(tableLayoutPosition)) {
    ...
    fun updateAtPosition(positions: List<TableLayoutPosition>, playerPosition: Int, data: T?) {
        updateAtPosition(positions[playerPosition], data)
    }
